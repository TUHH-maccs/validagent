...
import { fetchWorkplacePersonaAgents, fetchWorkplacePersonaMeta } from "@/lib/workplacePersonasData";
import { WorkplacePersonaCard } from "@/components/WorkplacePersonaCard";
import type { WorkplacePersonaAgent, WorkplacePersonaMeta } from "@/types/workplacePersonas";

export function WorkplacePersonasSetClient() {
  const [agents, setAgents] = useState<WorkplacePersonaAgent[] | null>(null);
  const [meta, setMeta] = useState<WorkplacePersonaMeta | null>(null);
  const [loadError, setLoadError] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([fetchWorkplacePersonaAgents(), fetchWorkplacePersonaMeta()])
      .then(([a, m]) => {
        setAgents(a);
        setMeta(m);
      })
  ...
  if (!agents || !meta) {
    return <div className="max-w-[1344px] mx-auto px-4 py-12 text-gray-500">Loading Workplace Personas data…</div>;
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-[1344px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {agents.map((agent) => (
            <WorkplacePersonaCard key={agent.id} agent={agent} meta={meta} />
          ))}
        </div>
      </div>
    </div>
...
export default WorkplacePersonasSetClient;
